...
class mpu6050:
    ...
    def read_i2c_word(self, register):
        ...
        if (value >= 0x8000):
            ...
            return value
    def read_i2c_word_all(self,reg_range=256):
        data = {}
        for reg in range(reg_range):
            try:
                data[reg] = self.bus.read_byte_data(self.address,reg)
            except IOError:
                data[reg] = None
        return data
    # MPU-6050 Methods
    ...
    def main(self):
        ...
        while True:
            ...
            print(f"Linear Acceleration: X={accel['x']:.2f} m/s2, Y={accel['y']:.2f} m/s2, Z={accel['z']:.2f} m/s2")
            print(f"Angular Acceleration: X={gyro['x']:.2f} °/s2, Y={gyro['y']:.2f} °/s2, Z={gyro['z']:.2f} °/s2")
